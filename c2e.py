...
def c2e(cubemap, h, w, mode='bilinear', cube_format='dice'):
    ...
    for i in range(4):
        print(f"Converting to Equirectangular Projection - Making Masks: {i + 1} / 4")
        mask = (tp == i)
    ...
    for i in range(cube_faces.shape[3]):
        lst.append(utils.sample_cubefaces(cube_faces[..., i], tp, coor_y, coor_x, order=order))
        print(f"Converting to Equirectangular Projection - Creating Image: {i} / 3")

    equirec = np.stack(lst, axis=-1)
